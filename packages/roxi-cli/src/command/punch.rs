...
use roxi_lib::{types::Address, util::init_logging};
...
pub struct Args {
    ...
    /// Address.
    #[clap(short, long, help = "Address to punch.")]
    pub address: String,
}
...
pub async fn exec(args: Args) -> anyhow::Result<()> {
    ...
    let addr = Address::try_from(args.address)?;

    let mut client = Client::new(config).await?;
    if let Err(e) = client.nat_punch(addr).await {
